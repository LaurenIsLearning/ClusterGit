...
export function validatePasswordAuthEmail(email) {
  ...
  if (!normalized) return "Please fill in all fields";
  if (!isValidEmailFormat(normalized)) return "Enter a valid email address";
  if (!isAllowedSchoolEmail(normalized)) {
    return "Use your school email for password sign-in or sign-up";
  }
